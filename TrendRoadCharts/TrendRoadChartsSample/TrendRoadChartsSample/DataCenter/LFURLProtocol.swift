...
class LFURLProtocol: URLProtocol, URLSessionDataDelegate, URLSessionTaskDelegate {
    ...
    override class func canInit(with request: URLRequest) -> Bool {
        ...
        if ["/all","details.php?issue"].index(of: urlString) == NSNotFound {
            return false
        }
        return true
    }
    ...
    override func startLoading() {
        ...
        //判断是否有本地缓存
//        let possibleCachedResponse = nil //self.cachedResponseForCurrentRequest()
...
//        } else {
 ...
 //       }
    ...
    //MARK: - URLSessionDataDelegate相关的代理方法
    ...
    func saveCachedResponse () {
        print("+++++ 将获取到的数据缓存起来 +++++")
        let saveTxt = String(data: self.receivedData! as Data, encoding: .utf8)
        print("\(String(describing: saveTxt))")
        /*
        //获取管理的数据上下文 对象
